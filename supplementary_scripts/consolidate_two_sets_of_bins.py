...
for bin_1 in all_bin_pairs:
    ...
    for bin_2 in all_bin_pairs[bin_1]:
        ...
        if (bins_2_stats[bin_2][0]-bins_2_stats[bin_2][1]*5) > score:
            cmd = "cp " + sys.argv[2] + '/' + bin_2 + " " + \
                sys.argv[5] + "/bin." + str(bin_ct) + ".fa"
            new_summary_file += "bin." + \
                str(bin_ct) + "\t" + \
                "\t".join(bins_2_summary[bin_2].split("\t")[1:])
            found_better = True
    if found_better == False:
        new_summary_file += "bin." + \
            str(bin_ct) + "\t" + \
            "\t".join(bins_1_summary[bin_1].split("\t")[1:])
        cmd = "cp " + sys.argv[1] + '/' + bin_1 + " " + \
            sys.argv[5] + "/bin." + str(bin_ct) + ".fa"
    os.system(cmd)
    bin_ct += 1
...
for bin_2 in bins_2_stats:
    if bins_2_stats[bin_2][0] < c or bins_2_stats[bin_2][1] > x:
        continue
    if bin_2 in bins_2_matches:
        continue
    new_summary_file += "bin." + \
        str(bin_ct) + "\t" + "\t".join(bins_2_summary[bin_2].split("\t")[1:])
    cmd = "cp " + sys.argv[2] + '/' + bin_2 + " " + \
        sys.argv[5] + "/bin." + str(bin_ct) + ".fa"
    os.system(cmd)
    bin_ct += 1
