...
for file_name in sys.argv[3:]:
    ...
    if len(data[bin_set]) > max_x:
        max_x = len(data[bin_set])
...
plt.yticks(list(range(min_completion, 105, 10)), [
           str(x) + "%" for x in range(min_completion, 105, 10)], fontsize=14)
...
# Make sure your axis ticks are large enough to be easily read.
plt.yticks(list(range(-0, max_contamination+1, 1)),
           [str(x) + "%" for x in range(-0, max_contamination+1, 1)], fontsize=14)
plt.xticks(fontsize=14)
